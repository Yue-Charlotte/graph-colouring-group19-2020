...
public class Group19Phase2 {
	...
	public static void main( String args[] ) {
		...
		int n = 100;
		int m = 30;
		ColEdge[] e = generateRandomGraph(n, m);
		...
		calculateChromaticNumber(G, n, m);
	}
	...
	public static ColEdge[] generateRandomGraph(int n, int m){
		...
		for(int i = 0; i < m; i++){
			...
			// Ensure we don't have this edge already
			for(int j = 0; j < i; j++) {
				if ((e[j].u == e[i].u && e[j].v == e[i].v) || (e[j].u == e[i].v && e[j].v == e[i].u)) {
					i--;
					DEBUGprintCurrentEdge = false;
					if (DEBUG) System.out.println("Redoing edge...");
					break;
				}
			}
			
			if (DEBUGprintCurrentEdge && DEBUG){
    ...
    public static int calculateChromaticNumber(Graph G, int n, int m){
		...
		System.out.println("Lower bound: " + Group19Phase2.lower);
		System.out.println("Upper bound: " + Group19Phase2.upper);
		System.out.println("Chromatic: " + Group19Phase2.chrom);
		System.out.println();	
		
		return Group19Phase2.chrom;
	}
