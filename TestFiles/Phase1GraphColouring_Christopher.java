...
public class Phase1GraphColouring
    {
        ...
        //---<<code here>>----
				public static void number(Graph G, int c){
					int v = G.GetVertexCount();
					int[] v_colours = new int[v];
					
					if (vertexCheck(G, c, v_colours, 0)){
						System.out.println("Solution " + c + " valid.");
					}else{
						System.out.println("Solution " + c + " invalid.");
					}
				}
        ...
        //---<<code here>>----
				public static boolean vertexCheck(Graph G, int c, int[] v_colours, int v){
					if (v == v_colours.length){
						return true;
					}
					
					// let's say we have c = 2. This means we want either colours 1 or 2. v_colours is initializes as all 0, which means no colouring has been made. 
					for(int i = 1; i <= c; i++){
						if (colourCheck(G, v, v_colours, i)){
							v_colours[v] = i;
							break;
						}
					}
					
					// same reason as comment above: 0 is "uncoloured". If we find 0, we can return false.
					if (vertexCheck(G, c, v_colours, v+1)){
						if (v == 0){
							for(int j = 0; j < v_colours.length; j++){
								if (v_colours[j] == 0){	
									return false;
								}
							}
        ...
        //Method to check if connected vertices don't have the same colour
            ...
            //else return true
        //---<<code here>>----
				public static boolean colourCheck(Graph G, int v, int[] v_colours, int c){
					for(int i = 0; i < G.GetVertexCount(); i++){
						if (G.HasEdge(v, i) && c == v_colours[i]){
							return false;
						}
					}
					return true;
